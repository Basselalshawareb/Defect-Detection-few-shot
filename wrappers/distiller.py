...
from mmrazor.models.builder import DISTILLERS,build_loss
from mmrazor.models.distillers.base import BaseDistiller
...
class SingleTeacherDistillerFewshot(BaseDistiller):
    ...
    # def build_teacher(self, cfg, ckpt):
    ...
    def exec_teacher_forward(self, data,bassel=0):
        ...
        if self.teacher_trainable:
            ...
            with torch.no_grad():
                ...
                ### gt_bboxes
                ### gt_labels
                output = self.teacher(**data,bassel=bassel)
                # if bassel!=2:
                #     print("Teacher loss")
                #     print(output['loss_cls'].detach().item())
                # return_loss=True
        return output
    ...
    def train(self, mode=True):
        ...
        super(SingleTeacherDistillerFewshot, self).train(mode)
    ...
    def compute_distill_loss(self, data=None):
        ...
        for i, component in enumerate(self.components):
            ...
            # #I MADE CHAAAAAAAAAAAAAANGES HEEEEEEEEEEEEEERE
            # temp = teacher_outputs[:,:-1].copy()
            # zr_temp = torch.zeros((256,3), requires_grad=True)
            # stack1 = torch.hstack((temp,zr_temp))
            # teacher_outputs_new = torch.hstack((stack1,teacher_outputs[:,-1].copy()))
            # teacher_outputs = teacher_outputs_new
            # print("TEEEEACHER OUTPUT")
